...
{
  // printf("StNegativeBinomial  (npp, k, x, eff, trig. bias) = (%1.3f, %1.3f, %1.3f, %1.3f, %1.3f)\n", mNpp, mK, mX, mEfficiency, mTriggerBias);
  // if(mIsConstEfficiency){
  //   printf("StNegativeBinomial  Use constant efficiency\n");
  // }
  // else{
  //   printf("StNegativeBinomial  Use multiplicity dependent efficiency\n");
  // }
...
Int_t StNegativeBinomial::GetMultiplicity(const Double_t npart, const Double_t ncoll) const
{
  ...
  for(Int_t im=0; im<2*multIdeal; im++){
    if( gRandom->Uniform(0,2) <= efficiency ){
      mult++;
    }
  }
